...
WEIGHTS_DIR = './params/inceptionresnetv2_f'
if not os.path.exists(WEIGHTS_DIR):
    os.makedirs(WEIGHTS_DIR)

WEIGHTS_PATH = os.path.join(WEIGHTS_DIR, 'weights_inceptionresnetv2_f_299.h5')
...
#  TensorBoard(log_dir='./log/inceptionresnetv2', write_images=True)]

input_tensor = Input(shape=(config.IMAGE_SIZE, config.IMAGE_SIZE, 3))

base_model = InceptionResNetV2(input_shape=(config.IMAGE_SIZE, config.IMAGE_SIZE, 3),
...
df_test.to_csv('./results/submission_inceptionresnetv2_f_299.csv', index=False)
